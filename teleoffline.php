...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $placeOfMeeting = $_POST['placeOfMeeting'];
    $date = $_POST['date'];
    $time = $_POST['time'];

    // Fetch doctor's name and fees according to the selected area
    $sql = "SELECT e_id,e_name, fees FROM employees WHERE area = '$areaOfDisease' and mode='Offline'";
    ...
    VALUES ($id,'$name','$areaOfDisease',$did,'$doctorName','Offline','$placeOfMeeting','$date','$time','Upcoming',$fees,'$description')";

    if ($conn->query($insertSql) === TRUE) {
        echo '<script>alert("Appointment Scheduled Successfully ! Check In Your Appointments And E-Mail.");</script>';
        ini_set('SMTP', 'smtp.gmail.com');
        ini_set('smtp_port', 587);
        $subject = "Offline Visit Scheduled For Patient ID $id";
        $body = "Dear $firstName,\n\nThis is to confirm that your offline doctor visit appointment has been successfully scheduled through Wellness Web. We are pleased to inform you that $doctorName has been appointed for your visit.
            ...
            function showFees() {
                ...
                switch (areaOfDisease) {
                    case "select":
                        ...
                        break;
                    case "Cardiology":
                        fees = "₹ 1000"; // Replace with actual fees for Cardiology
                        break;
                    case "Dermatology":
                        fees = "₹ 600"; // Replace with actual fees for Dermatology
                        break;
                    case "Endocrinology":
                        fees = "₹ 900"; // Replace with actual fees for Endocrinology
                        break;
                    case "Gastroenterology":
                        fees = "₹ 700"; // Replace with actual fees for Gastroenterology
                        break;
                    case "Hematology":
                        fees = "₹ 1000"; // Replace with actual fees for Hematology
                        break;
                    case "Neurology":
                        fees = "₹ 1200"; // Replace with actual fees for Neurology
                        break;
                    case "Oncology":
                        fees = "₹ 1800"; // Replace with actual fees for Oncology
                        break;
                    case "Orthopedics":
                        fees = "₹ 1200"; // Replace with actual fees for Orthopedics
                        break;
                    case "Pediatrics":
                        fees = "₹ 700"; // Replace with actual fees for Pediatrics
                        break;
                    case "Pulmonology":
                        fees = "₹ 900"; // Replace with actual fees for Pulmonology
                        break;
                    case "Rheumatology":
                        fees = "₹ 1000"; // Replace with actual fees for Rheumatology
                        break;
                    case "Urology":
                        fees = "₹ 1200"; // Replace with actual fees for Urology
                        break;
                    case "Psychiatry":
                        fees = "₹ 700"; // Replace with actual fees for Psychiatry
                        break;
                    case "Ophthalmology":
                        fees = "₹ 600"; // Replace with actual fees for Ophthalmology
                        break;
                    case "Dentistry":
                        fees = "₹ 400"; // Replace with actual fees for Dentistry
                        break;
                    default:
